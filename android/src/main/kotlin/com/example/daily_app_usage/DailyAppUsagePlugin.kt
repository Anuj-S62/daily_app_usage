package com.example.daily_app_usage
...
/** DailyAppUsagePlugin */
class DailyAppUsagePlugin: FlutterPlugin, MethodCallHandler,ActivityAware{
  ...
  private val methodChannelName = "daily_app_usage.methodChannel"
  ...
  fun getUsageStats(@NonNull call: MethodCall, @NonNull result: Result) {
    handlePermissions()

    val usageStats = getCurrentUsageStats()

    if (usageStats != null) {
      result.success(usageStats)
    } else {
      result.error("UNAVAILABLE", "Usage Stats are not available.", "Unknown")
    }
  }
...
//        get package name and app name
  ...
  fun getCurrentUsageStats(): Map<String,Long> {
    ...
    var usageMap : MutableMap<String, Long> = mutableMapOf()

    if(manager!=null){
      val events : UsageEvents = manager.queryEvents(startTime, endTime)
        ...
        while(events.hasNextEvent()){
          ...
          if(packageName.equals(tar)){
            if(event.eventType == UsageEvents.Event.MOVE_TO_FOREGROUND){
              ...
              if(offset>0){
                duration += event.timeStamp - offset
                offset = 0
              }
              usageMap[app] = duration
            }
          }
        }
      }
